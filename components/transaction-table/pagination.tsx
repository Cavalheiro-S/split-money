...
function TransactionTablePagination({
  page,
  limit,
  totalPages,
  onChange,
    ...
    return (
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-gray-50 rounded-lg">
        <div className="flex items-center gap-2 text-sm text-gray-600">
          <span>
            Mostrando {filteredDataLength} de {totalItems || filteredDataLength}{" "}
            transações
          </span>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-600">
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-gray-50 rounded-lg">
      <div className="flex items-center gap-2 text-sm text-gray-600">
        {filteredDataLength !== undefined && totalItems !== undefined ? (
          <span>
            Mostrando{" "}
            {Math.min((page - 1) * (limit || 10) + 1, filteredDataLength)} a{" "}
            {Math.min(page * (limit || 10), filteredDataLength)} de {totalItems}{" "}
            transações
          </span>
        ) : (
          <span>
