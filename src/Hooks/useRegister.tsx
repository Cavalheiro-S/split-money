import { collection, deleteDoc, doc, getDocs, limit, onSnapshot, orderBy, query, QueryConstraint, QueryDocumentSnapshot, setDoc, SnapshotOptions, updateDoc, where } from "firebase/firestore";
...
export const useRegister = () => {
    ...
    const getRegisterByTypeFirestore = async (type: RegisterType) => {
        ...
        const registersQuery = registers.filter(register => register.type === type);
        return registersQuery;
    }
    ...
    const getRegistersLimit = async (limitResult = 10) => {
        ...
        //     orderBy("date", "asc"),
        //     limit(limitResult)]);
        const registersQuery = registers.slice(0, limitResult);
        return registersQuery;
    }
    ...
    const getRegistersByMonth = async (month: string) => {
        ...
        const registersQuery = registers.filter(register => {
            const registerMonth = moment(register.date).format("M");           
            return registerMonth == month;
        });
        return registersQuery;
    }
    ...
    const getRegistersLength = async () => {
        try {
            // const registersQuery = await getRegisterBase();
            const registersQuery = registers;
